...
class Application {
    constructor() {
        ...
        this.ftaDecimals = 8; // 8 DECIMALES
    ...
    async updateData() {
        ...
        try {
            ...
            // RÉCUPÉRATION DU MULTIPLICATEUR (IMPORTANT POUR AFFICHAGE RÉEL)
            try { 
            ...
            document.getElementById('bal-matic').innerText = parseFloat(ethers.formatEther(maticBal)).toFixed(4);
            ...
            // Si rate = 80 000 000 (8 décimales), float = 0.8
            this.currentRate = parseFloat(ethers.formatUnits(rate, this.ftaDecimals));
            ...
            if (this.swapDirection === 'USDT_TO_FTA') {
                ...
                document.getElementById('swap-rate').innerText = `1 FTA = ${inverseRate.toFixed(2)} USDT`;
            }
    ...
    updateSwapIcons() {
        const iconFrom = document.getElementById('swap-icon-from');
        const iconTo = document.getElementById('swap-icon-to');
        const isUsdtTo = this.swapDirection === 'USDT_TO_FTA';
        
        iconFrom.style.backgroundImage = `url('${isUsdtTo ? CONFIG.LOGO_USDT : CONFIG.LOGO_FTA}')`;
        iconTo.style.backgroundImage = `url('${isUsdtTo ? CONFIG.LOGO_FTA : CONFIG.LOGO_USDT}')`;
    }
    ...
    checkReferral() {
        ...
        if (ref && ethers.isAddress(ref)) {
            // Logic to bind ref if needed
        }
    }

    copyLink() { const val = document.getElementById('ref-link').value; if (!val || val === "Connectez-vous...") return; navigator.clipboard.writeText(val); this.showToast("Lien copié !"); }
    ...
    async renderShop(forceFetch = false) {
        ...
        try {
            ...
            for(let i=0; i<count; i++) {
                ...
                // Calcul Puissance RÉELLE (avec difficulté)
                const powerBN = BigInt(data.power.toString());
                const effectivePowerBN = (powerBN * this.currentMultiplier) / 1000000000000000000n;
                const power = parseFloat(ethers.formatUnits(effectivePowerBN, this.ftaDecimals)); 
    ...
    async buyMachine(id) {
        ...
        try {
            ...
            this.isLoadingShop = false; await this.renderShop(true);
            await this.checkMyMachines(); this.updateData();
    ...
    calcSwap() {
        ...
        // currentRate est 0.8
        if (this.swapDirection === 'USDT_TO_FTA') {
            res = val * this.currentRate;
        } else {
            res = val / this.currentRate;
        }
        document.getElementById('swap-to-in').value = res.toFixed(5);
        
        const details = document.getElementById('dex-details');
        details.style.display = val > 0 ? 'block' : 'none';
    }
