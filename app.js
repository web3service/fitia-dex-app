...
class Application {
  ...
  animateVisualizer() {
    ...
    this.vizBars.forEach((b, i) => {
      ...
      if (b.peak === undefined) b.peak = 0;
      b.height += (b.targetHeight - b.height) * 0.1;
      // Petit plancher lumineux de 3px pour un rendu actif même au repos
      const h = Math.max(b.height, 3);
      const x = i * w + 3, bw = w - 6, y = H - h;
      ctx.beginPath();
      // Barre aux coins arrondis (roundRect si disponible, rect sinon)
