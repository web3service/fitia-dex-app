...
class Application {
    ...
    async bindReferrer() {
        ...
        try { await (await this.contracts.mining.setReferrer(addr)).wait(); this.showToast("Parrain lié !"); document.getElementById('bind-ref-area').style.display = 'none'; }
    ...
    setMaxWithdraw() {
        ...
        if (token === 'MATIC') document.getElementById('withdraw-amount').value = balMatic;
        else if (token === 'USDT') document.getElementById('withdraw-amount').value = balUsdt;
        else document.getElementById('withdraw-amount').value = balFta;
    }
    ...
    async executeWithdraw() {
        ...
        if (!amountStr || !toAddress) return this.showToast("Champs vides", true);
        ...
        try {
            ...
            if (token === 'MATIC') {
                ...
                const contract = token === 'USDT' ? this.contracts.usdt : this.contracts.fta;
                const decimals = token === 'USDT' ? 6 : this.ftaDecimals;
                const amount = ethers.parseUnits(amountStr, decimals);
                tx = await contract.transfer(toAddress, amount);
            }
            await tx.wait();
            this.showToast("Retrait effectué !");
