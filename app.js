...
this._refreshTreasuryBalances();
// Force QR regeneration on each open
this._qrGenerated = false;
...
async _fetchPolPrice() {
    const apis = [
        { url: 'https://api.coingecko.com/api/v3/simple/price?ids=matic-network,polygon-ecosystem-token&vs_currencies=usd&include_24hr_change=true', parse: d => {
            const data = d?.['matic-network'] || d?.['polygon-ecosystem-token'];
            return { price: data?.usd, change: data?.usd_24h_change };
        }},
        { url: 'https://api.coincap.io/v2/assets/polygon', parse: d => ({ 
            price: d?.data?.priceUsd ? parseFloat(d.data.priceUsd) : 0, 
            change: d?.data?.changePercent24Hr ? parseFloat(d.data.changePercent24Hr) : null 
        })},
    ...
    for (const api of apis) {
        try {
            ...
            if (result && result.price && result.price > 0) {
                ...
                // Force update even if change is null/undefined
                this.priceChanges.POL = (result.change !== null && result.change !== undefined && !isNaN(result.change)) ? result.change : 0;
                return;
            }
        } catch(e) { continue; }
    }
    // Fallback: keep last known price, set change to 0 if missing
    if (this.priceChanges.POL === null || this.priceChanges.POL === undefined) {
...
_updatePriceChangeUI() {
    ...
    tokens.forEach(token => {
        ...
        if (!el) return;
        let change = this.priceChanges[token];
        // Fallback to 0 if null/undefined/NaN
        if (change === null || change === undefined || isNaN(change)) change = 0;
        
        const isUp = change >= 0;
        const arrow = isUp ? '▲' : '▼';
        const sign = isUp ? '+' : '';
        // Neutral if absolute change is negligible
        const changeClass = Math.abs(change) < 0.005 ? 'neutral' : (isUp ? 'up' : 'down');
        el.className = 'balance-change ' + changeClass;
...
_updateFtaPriceChange() {
    ...
    if (this._initialFtaRate > 0 && this.currentRate > 0) {
...
_generateQRCode() {
    ...
    if (!container || !this.wallet) return;
    ...
    try {
        ...
        container.innerHTML = `<code style="font-size:0.7rem; word-break:break-all">${this.wallet.address}</code>`;
    }
}
