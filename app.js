...
let tradeSide = 0;
let chartInterval;
let currentAsset = 2; // FTA
let lastClose = 1.25; 
let ftaCorrelation = 0.98;
...
window.execHomeSwap = async (isBuy) => {
    const amt = document.getElementById('hSwapIn').value; if(!amt) return toast("Montant requis", 'err');
    const inTok = document.getElementById('hSwapInTok').value;
    try {
        if(isBuy && inTok === 'usdt') { await (await contract.buyFTA(ethers.utils.parseUnits(amt, 6))).wait(); }
        else if(!isBuy && inTok === 'fta') { await (await contract.sellFTA(ethers.utils.parseUnits(amt, 8))).wait(); }
        else { toast("Utilisez USDT pour acheter ou FTA pour vendre", 'inf'); return; }
        toast("Échange réussi", 'succ'); loadUser();
...
async function loadUser() {
    if(!contract) return;
    try {
        const [uBal, fBal] = await Promise.all([ contract.internalBalances(user, usdtAddr), contract.internalBalances(user, ftaAddr) ]);
...
window.openTrade = async () => {
    const m = document.getElementById('tMargin').value; const l = document.getElementById('tLev').innerText.replace('x','');
    const a = document.getElementById('tradeAsset').value; const sl = document.getElementById('tSL').value || 0; const tp = document.getElementById('tTP').value || 0;
    if(!m) return toast("Marge requise", 'err');
    try { const tx = await contract.openPositionWithLimits(a, tradeSide, ethers.utils.parseUnits(m, 6), l, sl, tp); toast("Envoi...", 'inf'); await tx.wait(); toast("Position ouverte", 'succ'); loadUser(); } catch(e) { toast(e.reason, 'err'); }
};
...
async function loadPos() { if(!contract) return; const c = await contract.positionCount(); if(c==0) return; let html = ''; for(let i=c; i>Math.max(0, c-5); i--) { const p = await contract.positions(i); if(p[0].toLowerCase()===user.toLowerCase() && p[6]) { const type = p[2]==0 ? "LONG" : "SHORT"; const cls = p[2]==0 ? "" : "sh"; const m = parseFloat(ethers.utils.formatUnits(p[3],6)).toFixed(2); html += `<div class="pos-item ${cls}"><div class="pos-h"><span>#${i} ${type}</span><button class="btn-sm" onclick="closePos(${i})">X</button></div><div class="pos-d"><span>Marge: ${m}</span></div></div>`; } } document.getElementById('posList').innerHTML = html || '<div class="empty-state">Aucune</div>'; }
