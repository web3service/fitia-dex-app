...
const App = {
    ...
    async initContracts() {
        ...
        try {
            ...
            this.state.contracts.mining = new ethers.Contract(CONTRACT_ADDRESSES.FITIA_MINING, CONTRACT_ABI.FITIA_MINING, provider);
            this.state.contracts.fta = new ethers.Contract(CONTRACT_ADDRESSES.FTA_TOKEN, CONTRACT_ABI.ERC20, provider);
            this.state.contracts.usdt = new ethers.Contract(CONTRACT_ADDRESSES.USDT_TOKEN, CONTRACT_ABI.ERC20, provider);
    ...
    async loadOnChainData() {
        ...
        try {
            ...
            if (referrer !== ethers.ZeroAddress) this.state.referralCode = referrer;
            await this.loadContractData();
            this.updateBalances();
            this.updateStats();
            this.renderMyRigs();
    ...
    startMiningSimulation() { setInterval(() => this.updateStats(), 1000); },
    ...
    async claim() {
        if (!this.state.connected) { this.showToast('⚠️ Connectez votre wallet', 'error'); return; }
        if (this.state.pending <= 0) { this.showToast('⚠️ Rien à réclamer', 'error'); return; }
        try {
    ...
    saveState() {
        const stateToSave = { address: this.state.address, connected: this.state.connected, referralCode: this.state.referralCode };
